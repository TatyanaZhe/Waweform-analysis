// Analys.cpp: определ€ет точку входа дл€ консольного приложени€.
//
...
int _tmain(int argc, _TCHAR* argv[])
{
	...
	while (!file.eof()) //цикл, выт€гивающий данные из файла в массив структур типа data под названием arr.
	{                   //в каждом элементе массива хранитс€ врем€ измерени€ и значение амлитуды, соответствующее времени
		if (file.get() == ':')
		{
			file.get();
			char sign = file.get();
	...
	for (int i = 0; i < t-2;i++) //цикл, выт€гивающий точки максимума из массива значений в массив res
	{
		if (arr[i].amp < arr[i+2].amp && arr[i+1].amp <= arr[i+2].amp //условие экстремума (провер€ем окрестность в два измерени€)
			&&arr[i+3].amp <= arr[i+2].amp && arr[i+4].amp < arr[i+2].amp)
		{
			res[c++] = arr[i+2];
			if (arr[i + 3].amp == arr[i + 2].amp)
			{
				i += 2; //перекидываем индекс вперед на два значени€, в случае, если экстремум дублируетс€ в соседней от него точке
			}
		}
	}
	file.close();
	...
	//критерий апериодичности (почти на глаз, проверкой превышени€ тангенса угла наклона некой контрольной величины
	//(угла наклона пр€мой между 1 и 2 точками))
	...
	//логарфмируем амлитуды и переместим начало координат дл€ ћЌ 
	...
	//ѕродолжаем вывод
	cout << "DEADPOINT NUMBER (WARNING, FLUCTUATION MAY OCCUR) :" << endPoint << endl; //точка, в которой начинаетс€ апериодичность
